...
fn run_bfs_deflate(alloc: std.mem.Allocator, io: std.Io) !void {
    ...
    for (1..200) |depth| {
        ...
        while (todo_r.pop()) |b| : (i += 1) {
            const tilecount = @popCount(b.tiles) + (b.pocket >> 1) + 1;
            if (i % 1000000 == 0) std.debug.print("progress = {}% {}/{} total={}\n", .{ (i * 100) / todo.len, i, todo.len, seen.len + todo.len });
            if (b.pocket == 1) {
                _ = check_solution(b, tilecount);
            }
            if (tilecount < 20) continue; // not enough tiles left for Eus
            for (std.enums.values(Action)) |a| {
        ...
        while (seen_r.hasNext() and p_i < parts.len) {
            ...
            while (p_ii + 1 < parts[p_i].items.len and parts[p_i].items[p_ii] == parts[p_i].items[p_ii + 1]) {
                dupes += 1;
                p_ii += 1;
            }
        ...
        while (todo_r.pop()) |b| : (i += 1) {
            ...
            for (std.enums.values(Action)) |a| {
                if (do_action(b, a)) |result| {
                    ...
                    if (b.gray == 4 or b.gray == 3 or b.gray == 10 or b.gray == 16 or b.gray == 34 or b.gray == 29 or b.gray == 5 or b.gray == 11) continue;
                    const bin: u16 = @intCast(@as(u80, @bitCast(result)) >> 64);
                    const val: u64 = @truncate(@as(u80, @bitCast(result)));
                    try parts[bin].append(alloc, val);
                    //
...
fn run_bfs_compress(alloc: std.mem.Allocator, io: std.Io) !void {
    // https://www.snellman.net/blog/archive/2018-07-23-optimizing-breadth-first-search/
    // Invariant: todo contains one depth
    var todo = compressedStream.init();
    var todow = todo.writer();
    ...
    for (1..200) |depth| {
        ...
        std.debug.print("\n\ndepth {}  todo {}\n", .{ depth, todo.len });
        const insert_start = Timestamp.now(io, .awake);
        var next = try std.ArrayList(Board).initCapacity(alloc, todo.len * 2);
        var i: usize = 0;
        var todo_r = todo.reader();
        //std.debug.print("reader: {} items: {any}\n", .{ todo_r, todo_r.stream.*.arr.items });
        while (todo_r.pop()) |b| : (i += 1) {
            //const b: Board = todo_r.pop().?;
            //if (i >= 10) return error.Exit;
            const tilecount = @popCount(b.tiles) + (b.pocket >> 1) + 1;
            if (i % 1000000 == 0) std.debug.print("progress = {}% {}/{} total={}\n", .{ (i * 100) / todo.len, i, todo.len, seen.len + todo.len });
            //std.debug.print("i {}  board {} tiles {}\n", .{ i, b, tilecount });
            if (b.pocket == 1) {
                _ = check_solution(b, tilecount);
            }
            if (tilecount < 20) continue; // not enough tiles left for Eus
        ...
        std.debug.print("sorting frontier\n", .{});
        const sort_start = Timestamp.now(io, .awake);
        // empirically: 19s pdq sort vs 20s block sort at depth=27
        std.sort.pdq(Board, next.items, {}, boardLessThan);
        const sort_dur = sort_start.untilNow(io, .awake).toNanoseconds();
        // merge sorted next (depth=d) with sorted seen (depth<d)
        // only sorted according to boardLessThan func (depth info is forgotten)
        std.debug.print("allocating for merge\n", .{});
        ...
        std.debug.print("merging\n", .{});
        const merge_start = Timestamp.now(io, .awake);
        while (seen_r.hasNext() and next_i < next.items.len) {
            // clear duplicates from sorted
            while (next_i + 1 < next.items.len and next.items[next_i] == next.items[next_i + 1]) {
                dupes += 1;
                next_i += 1;
            }
            const seen_r_peek = seen_r.peek().?;
            if (seen_r_peek == next.items[next_i]) {
                try seen_w.write(seen_r.pop().?, alloc);
                next_i += 1;
            } else if (boardLessThan({}, seen_r_peek, next.items[next_i])) {
                try seen_w.write(seen_r.pop().?, alloc);
            } else {
                try todo_w.write(next.items[next_i], alloc);
                try seen_w.write(next.items[next_i], alloc);
                next_i += 1;
            }
        }
        while (seen_r.hasNext()) {
...
fn run_bfs_sortedmerge(alloc: std.mem.Allocator) !void {
    // TODO sorting is very slow
    // https://www.snellman.net/blog/archive/2018-07-23-optimizing-breadth-first-search/
    // Invariant: todo contains one depth
    var todo = try std.ArrayList(Board).initCapacity(alloc, 1);
    try todo.append(alloc, b053);
    // Invariant: seen is sorted by (state)  [ignoring depth]
    var seen = try std.ArrayList(Board).initCapacity(alloc, 1);
    try seen.append(alloc, b053);
    //
    //var parent = std.ArrayList(Action).init(alloc);
    //try parent.append(.Z);  // ULDR denote previous *facing* direction before move
    //defer parent.deinit()
    //var depth_indices = std.ArrayList(usize).initCapacity(alloc, 200);
    //try depth_indices.append(0);
    for (1..200) |depth| {
        //try depth_indices.append(seen.len);
        std.debug.print("\n\ndepth {}  todo {}\n", .{ depth, todo.items.len });
        var next = try std.ArrayList(Board).initCapacity(alloc, todo.items.len * 2);
        for (todo.items, 0..) |b, i| {
            const tilecount = @popCount(b.tiles) + (b.pocket >> 1) + 1;
            if (i % 1000000 == 0) std.debug.print("progress = {}% {}/{} total={}\n", .{ (i * 100) / todo.items.len, i, todo.items.len, seen.items.len + todo.items.len });
            if (b.pocket == 1) {
                _ = check_solution(b, tilecount);
            }
            if (tilecount < 23) continue; // not enough tiles left for Eus
            for (std.enums.values(Action)) |a| {
                if (do_action(b, a)) |result| {
                    try next.append(alloc, result);
                }
        ...
        todo.deinit(alloc);
        std.debug.print("sorting frontier\n", .{});
        std.sort.block(Board, next.items, {}, boardLessThan);
        // merge sorted next (depth=d) with sorted seen (depth<d)
        // only sorted according to boardLessThan func (depth info is forgotten)
        std.debug.print("allocating for merge\n", .{});
        // TODO these should be out-streams?
        var out_todo = try std.ArrayList(Board).initCapacity(alloc, next.items.len);
        var out_seen = try std.ArrayList(Board).initCapacity(alloc, seen.items.len);
        var seen_i: usize = 0;
        var next_i: usize = 0;
        std.debug.print("merging\n", .{});
        while (seen_i < seen.items.len and next_i < next.items.len) {
            // clear duplicates from sorted
            while (next_i + 1 < next.items.len and next.items[next_i] == next.items[next_i + 1]) {
                next_i += 1;
            }
            if (seen.items[seen_i] == next.items[next_i]) {
                try out_seen.append(alloc, seen.items[seen_i]);
                seen_i += 1;
                next_i += 1;
            } else if (boardLessThan({}, seen.items[seen_i], next.items[next_i])) {
                try out_seen.append(alloc, seen.items[seen_i]);
                seen_i += 1;
            } else {
                try out_todo.append(alloc, next.items[next_i]);
                try out_seen.append(alloc, next.items[next_i]);
                next_i += 1;
            }
        }
        while (seen_i < seen.items.len) {
            try out_seen.append(alloc, seen.items[seen_i]);
            seen_i += 1;
        }
        while (next_i < next.items.len) {
        ...
        std.debug.print("done merge, deallocating\n", .{});
        next.deinit(alloc);
        seen.deinit(alloc);
        seen = out_seen;
        todo = out_todo;
    }
    std.debug.print("finished bruteforcing with sortedmerge\n", .{});
}
...
fn run_bfs_hashmap(alloc: std.mem.Allocator) !void {
    var q = try std.Deque(Board).initCapacity(alloc, 10_000_000);
    try q.pushBack(alloc, b053);
    var seen = std.AutoHashMap(Board, void).init(alloc);
    var counter: u64 = 0;
    while (q.popFront()) |b| {
        const tilecount = @popCount(b.tiles) + (b.pocket >> 1);
        if (counter % 10000 == 0) std.debug.print("counter={} seen={} qlen={} tilecount={}\n", .{ counter, seen.count(), q.len, tilecount });
        counter += 1;
        if (b.pocket == 1) {
            _ = check_solution(b, tilecount);
        }
        ...
        for (std.enums.values(Action)) |a| {
            if (do_action(b, a)) |result| {
                if (seen.contains(result)) continue;
                try seen.put(result, {});
                try q.pushBack(alloc, result);
            }
        }
    }
    std.debug.print("finished bruteforcing with hashmap\n", .{});
}
